...
- You may run non-destructive maintenance scripts under `{{HOOKS_DIR}}/` (for example
  ...
  `{{PYTHON_COMMAND}} {{HOOKS_DIR}}/manage_memory.py list` to inspect entries). Require
  approval before running `manage_memory.py deprecate|restore|delete`, and do not modify
  files outside the memory scopes.
