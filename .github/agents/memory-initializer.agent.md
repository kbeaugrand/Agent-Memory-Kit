...
- Read `.aimem/memory/TEMPLATE.md` before writing memory and fill memory according to its
...
- Seed or propose `.aimem/memory/project.md` entries with durable, team-shared rules,
...
- Seed `.aimem/memory/session/current.md` only with short notes that are useful for the current setup
  session and should not become permanent project facts.
- If user preferences are explicitly provided, suggest adding them to `~/.aimem/memory/user.md`;
...
- For durable memory outside an explicit initialization request, recognize the lesson as
  it emerges — a user correction or stated rule, a confirmed fix, a verified command, or a
  validated decision — and present a memory candidate with scope, action, target, reason,
  and exact proposed content; require explicit approval before writing.
- Prefer the shared MCP memory service when available: use `memory_search` and
  ...
  is unavailable.
- Use `record_memory.py` fields from `.aimem/memory/TEMPLATE.md` (`scope`, `topic`, `kind`,
...
- Only write to memory files when explicitly invoked for initialization, after explicit
  ...
  - `~/.aimem/memory/user.md` when the user explicitly asks for a user-scoped preference
  - `.aimem/memory/agents/<agent>.md` when seeding memory for a specific agent
